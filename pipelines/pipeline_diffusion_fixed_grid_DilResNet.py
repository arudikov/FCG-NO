...
def get_DilResNet(key, features_train, grid):
    ...
    optimization_specification = {
        ...
        "make_step": lambda carry, indices, A, r, error, optim: make_step_scan(carry, indices, A, r, error, optim),
        "N_epochs": N_epoch,
...
@eqx.filter_jit
def make_step_scan(carry, indices, A, r, error, optim):
...
def save_data(model, times, R, history, path, experiment_name):
    data = {
        "times": times,
        "R_model": R,
        "history": history, 
...
def main(model_type, train_generation, grid, samples_div, N_repeats, m_max, path):
    ...
    if model_type == 'DilResNet':
        ...
        if train_generation == 'FCG':
            ...
            for j in tqdm(range(N_samples//2)):
                _, R, X, _, _ = FCG(A_train[j*2:(j+1)*2], rhs_train[j*2:(j+1)*2], model=model_, N_iter=N_repeats-1, m_max=m_max, optimization_specification=optimization_specification, count_values=False, j=j)
        ...
        N_iter = min(int(grid*4), 400)

        R, values = [], []
        times = []
        count_values = False
        for j in tqdm(range(N_samples//4)):
            _, R_, _, values_, times_ = FCG(A_test[j*4:(j+1)*4], rhs_test[j*4:(j+1)*4], model=model, N_iter=N_iter, m_max=m_max, optimization_specification=optimization_specification, count_values=count_values, j=j)
            R.append(R_)
    ...
    save_data(model, times, R, history, path, f'{model_type}_{train_generation}_{grid}_{samples_div}_{N_repeats}_{m_max}')
...
if __name__ == "__main__":
    ...
    grids = [32, 64, 128]
    path = f'./Elliptic/DilResNet/notay_loss_'
