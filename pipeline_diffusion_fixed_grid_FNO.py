...
from architectures import DilResNet, FNO
...
def get_FNO(key, features_train, grid):
    ...
    if grid <= 64:
        ...
        N_features = int(grid // 1.5)
        
    N_layers = 4
    N_features_out = 1
    N_modes = 16
    N_features_ = [input_shape, N_features, N_features_out]
    
    if D == 1:
        x = jnp.expand_dims(jnp.linspace(0, 1, features_train.shape[-1]), 0)
        N_features_[0] = N_features_[0] + x.shape[0]
        model = FNO.FNO1D(N_layers, N_features_, N_modes, key)
    elif D == 2:
        x = jnp.linspace(0, 1, features_train.shape[-1])
        x = jnp.stack(jnp.meshgrid(x, x), 0)
        N_features_[0] = N_features_[0] + x.shape[0]
        model = FNO.FNO2D(N_layers, N_features_, N_modes, key)
...
def main(model_type, train_generation, grid, samples_div, N_repeats, m_max, path):
    ...
    if model_type == 'FNO':
        ...
        model_data, optimization_specification = get_FNO(random.PRNGKey(40), residuals[0][None, ...], grid)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, help='model type') # FNO, Id
    ...
    grids = [32, 64, 128]
    path = f'./Elliptic/FNO/notay_loss_'
    # path = f'./Elliptic/l2_loss_'
    m_max = 20
